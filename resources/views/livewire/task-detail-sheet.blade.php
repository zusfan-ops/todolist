<div x-data="{
    ...
    @if ($task)
        <div wire:click.self="close" class="absolute inset-0 bg-ink-900/50 flex items-end z-20">
            <div class="bg-white w-full rounded-t-3xl max-h-[85%] overflow-y-auto no-scrollbar p-6">
                <div class="w-12 h-1.5 bg-ink-100 rounded-full mx-auto mb-4"></div>
                ...
                <div class="flex items-start justify-between gap-3">
                    <div>
                        ...
                        <h3 class="font-disp font-bold text-xl text-ink-900 mt-2">{{ $task->title }}</h3>
                    </div>
                ...
                @if ($task->description)
                    <p class="text-base text-ink-500 mt-3">{{ $task->description }}</p>
                @endif
                ...
                <div class="mt-6">
                    <h4 class="font-disp font-bold text-sm uppercase tracking-wider text-ink-700 mb-3">Checklist</h4>
                    <div class="space-y-2">
                        @foreach ($task->checklistItems as $item)
                            <label wire:key="check-{{ $item->id }}" class="flex items-center gap-3 bg-ink-50 rounded-xl px-4 py-3 cursor-pointer">
                                <input type="checkbox" wire:click="toggleChecklist({{ $item->id }})" @checked($item->is_done) class="w-5 h-5 accent-leaf-500">
                                <span class="text-base {{ $item->is_done ? 'line-through text-ink-300' : 'text-ink-900' }}">{{ $item->body }}</span>
                            </label>
                        @endforeach
                    </div>
                    <form wire:submit="addChecklistItem" class="flex gap-2 mt-3">
                        <input wire:model="newChecklistBody" placeholder="Tambah item…" class="flex-1 bg-ink-50 border border-ink-100 rounded-xl px-4 py-3 text-base">
                        <button type="submit" class="bg-ink-100 text-ink-700 text-sm font-disp font-bold px-4 rounded-xl">+</button>
                    </form>
                </div>
                ...
                <div class="grid grid-cols-2 gap-3 mt-6">
                    ...
                    <label class="bg-vest-500 text-ink-900 font-disp font-bold text-base py-3.5 rounded-xl text-center cursor-pointer" :class="capturing && 'opacity-50 pointer-events-none'">
                ...
                <div class="mt-6 mb-2">
                    <h4 class="font-disp font-bold text-sm uppercase tracking-wider text-ink-700 mb-3">Aktivitas</h4>
                    <div class="border-l-2 border-ink-100 pl-3 space-y-3">
