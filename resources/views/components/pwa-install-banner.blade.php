...
<div x-data="{
    ...
    <div class="min-w-0 flex-1">
        <p class="text-xs font-disp font-bold leading-snug">Instal KerjaKu ke layar utama</p>
        <p class="text-[11px] {{ $dark ? 'text-ink-300' : 'text-ink-500' }} leading-snug">Akses lebih cepat, bisa dipakai offline.</p>
    </div>
    <button @click="install" class="shrink-0 bg-vest-500 text-ink-900 text-xs font-disp font-bold px-3 py-2 rounded-lg">Instal</button>
    <button @click="dismiss" class="shrink-0 {{ $dark ? 'text-ink-300' : 'text-ink-300' }} text-lg leading-none px-1">&times;</button>
</div>
