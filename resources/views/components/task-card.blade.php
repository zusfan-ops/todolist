...
<div wire:key="task-{{ $task->id }}" data-id="{{ $task->id }}" data-position="{{ $task->position }}" data-progress="{{ $progress }}"
     ...
     class="bg-white rounded-xl border border-ink-100 p-4 cursor-pointer active:scale-[.98] transition-transform">
    <div class="flex items-center justify-between mb-2">
        ...
        @elseif ($task->priority === 'high')
            <span class="text-[10px] font-disp font-bold text-brick-500 bg-brick-100 px-2 py-0.5 rounded">TINGGI</span>
        @endif
    </div>
    <p class="text-base font-medium text-ink-900 leading-snug">{{ $task->title }}</p>
    <div class="prog-track mt-3">
        <div class="prog-fill {{ $done ? 'done' : '' }}" style="width: {{ $progress }}%"></div>
    </div>
    <div class="flex items-center justify-between mt-2">
        <span class="font-mono text-xs font-bold {{ $done ? 'text-leaf-500' : 'text-ink-700' }}">{{ $progress }}%</span>
        <div class="flex items-center gap-3">
            @if ($checklistTotal)
                <span class="font-mono text-xs text-ink-500">☑ {{ $checklistDone }}/{{ $checklistTotal }}</span>
            @endif
