...
@if ($vapidConfigured)
    <div x-data="{
        ...
        <div class="min-w-0 flex-1">
            <p class="text-xs font-disp font-bold leading-snug text-ink-900">Aktifkan pengingat</p>
            <p class="text-[11px] text-ink-500 leading-snug">Dapat notifikasi saat task jatuh tempo atau timer lupa di-stop.</p>
        </div>
        <button @click="enable" :disabled="enabling" class="shrink-0 bg-vest-500 text-ink-900 text-xs font-disp font-bold px-3 py-2 rounded-lg disabled:opacity-50">
            <span x-text="enabling ? '…' : 'Aktifkan'"></span>
        </button>
        <button @click="dismiss" class="shrink-0 text-ink-300 text-lg leading-none px-1">&times;</button>
    </div>
@endif
